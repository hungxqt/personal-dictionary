...
async function runDeckMutation(mutator) {
  ...
  const mutationResult = (await mutator({ deckStore, metaStore, currentRevision })) || {};
...
export async function bulkUpsertDeckItems(items) {
  ...
  const result = await runDeckMutation(({ deckStore }) => {
    ...
    return {
      items: sanitizedItems.map((item) => toPublicDeckItem(item)).filter(Boolean)
    };
  });
...
export async function writeDeckItems(items) {
  return await replaceDeckItems(items);
}
...
export async function chooseCustomDatabaseFile() {
  return await chooseDatabaseSyncFile();
}

export async function useDefaultDatabaseLocation() {
  await writeMetaValues({
    [META_KEY_SYNC_FILE_HANDLE]: null,
    [META_KEY_SYNC_FILE_NAME]: ""
  });
}
