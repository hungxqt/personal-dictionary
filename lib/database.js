...
function normalizeStarredValue(value) {
...
function sanitizeDeckItem(rawItem) {
  ...
  const starred = normalizeStarredValue(rawItem.starred ?? rawItem.isStarred ?? rawItem.favorite ?? rawItem.favourite);
...
function toPublicDeckItem(rawItem) {
  ...
  return {
    ...
    ...(sanitized.starred ? { starred: true } : {})
  };
}
...
export function createDeckItem({ sourceText, translatedText, sourceLang, targetLang, starred = false }) {
  const normalizedStarred = normalizeStarredValue(starred);
  return {
    ...
    ...(normalizedStarred ? { starred: true } : {})
  };
}
