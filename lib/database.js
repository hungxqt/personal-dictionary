...
async function saveToStorageFallback(db, options = {}) {
  const { onlyIfChanged = false } = options;
  const nextDeck = db.deck || [];

  if (onlyIfChanged) {
    const current = await loadFromStorageFallback();
    if (JSON.stringify(current.deck) === JSON.stringify(nextDeck)) {
      return;
    }
  }

  await chrome.storage.local.set({ [DECK_KEY]: nextDeck });
}
...
export async function readDeckItems() {
  ...
  if (meta.mode === "custom" && meta.fileHandle) {
    ...
    try {
      ...
      await saveToStorageFallback(db, { onlyIfChanged: true });
      return db.deck;
    } catch {
      const fallback = await loadFromStorageFallback();
      return fallback.deck;
